...
def send_risk_notification(
    ...
    return asyncio.run(
        process_risk_notification(
            user_id, risk_level, risk_category, message_summary, trace_id
        )
    )
...
async def process_risk_notification(
    ...
    Returns a status object for Trace / demo visibility.
    """
    ...
    try:
        async with async_session() as db:
            user_result = await db.execute(
                select(User.username).where(User.id == db_user_id)
            )
            ...
            return {
                "status": "persisted",
                "records": records,
                "webhook_status": "mixed",
                "error": None,
            }
    except Exception as e:
        logger.error(f"process_risk_notification failed: {e}")
        return {
            "status": "failed",
            "records": 0,
            "webhook_status": None,

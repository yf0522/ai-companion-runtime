...
test("final tools_used merge retry receipts by tool and invocation identity", () => {
  const traceId = begin("final-retry-receipts");
  useChatStore.getState().setToolResult({
    traceId,
    invocationId: "call-1",
    tool: "caretask",
  ...
  useChatStore.getState().finalizeMessage({
    traceId,
    messageId: "final-retry-receipts",
    ttftMs: 12,
    totalLatencyMs: 30,
    toolsUsed: [
      { tool: "caretask", invocation_id: "call-1", status: "failed" },
      { tool: "caretask", invocationId: "call-2", status: "success" },
    ],
    memoryUpdated: false,
  });
  ...
  assert.equal(tools?.some((tool) => tool.invocationId === undefined), false);
});
